...
const ChatList = ({ currentTab }) => {
  ...
  const getFetchUrl = () => {
    ...
    if (currentTab === "drafting") {
      ...
      const url = `https://juristo-backend-test.vercel.app/api/image-chat/${user._id}`;
      console.log("Fetching analysis chats from external API:", url);
      return url;
    } else {
      const url = `https://juristo-backend-test.vercel.app/api/chat/${user._id}`;
  ...
  const getDeleteUrl = (chatId) => {
    if (currentTab === "drafting") {
      ...
      const url = `https://juristo-backend-test.vercel.app/api/image-chat/${chatId}`;
      console.log("Deleting analysis chat from external API:", url);
      return url;
    } else {
      const url = `https://juristo-backend-test.vercel.app/api/chat/${chatId}`;
  ...
  const openChat = async (chat) => {
    ...
    try {
      ...
      if (currentTab === "drafting") {
        url = `https://juristo-backend-test.vercel.app/api/draftchats/chat/${chat.chatId}`;
      } else if (currentTab === "analysis") {
        url = `https://juristo-backend-test.vercel.app/api/image-chat/${chat.chatId}`;
      } else {
        url = `https://juristo-backend-test.vercel.app/api/chat/${chat.chatId}`;
      }
